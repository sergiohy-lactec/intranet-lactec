...
PROJECT_NAME=intranet-lactec
STACK_NAME=intranet-lactec-com-br
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://intranet-lactec.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t sergiohy-lactec/intranet-lactec-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t sergiohy-lactec/intranet-lactec-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name intranet-lactec-frontend-acceptance --link intranet-lactec-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d sergiohy-lactec/intranet-lactec-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name intranet-lactec-backend-acceptance -d sergiohy-lactec/intranet-lactec-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop intranet-lactec-frontend-acceptance
	@docker stop intranet-lactec-backend-acceptance
